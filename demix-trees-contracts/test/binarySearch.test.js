...
describe('findArrayLength', () => {
  ...
  beforeEach(async function () {
    const [operator, demixProxy] = await ethers.getSigners()
    ...
    demixTrees = await DeMixTrees.deploy(operator.address, publicArray.address, {
      depositsFrom: 3,
      depositsStep: 3,
      withdrawalsFrom: 2,
      withdrawalsStep: 2,
    })
    await demixTrees.initialize(demixProxy.address, publicArray.address)
  })

  it('should work for even array', async () => {
    const depositsLength = await demixTrees.findArrayLengthMock(
      publicArray.address,
      'deposits(uint256)',
      4,
      2,
