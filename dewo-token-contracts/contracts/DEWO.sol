...
contract DEWO is ERC20("DecentraWorld", "DEWO"), ERC20Burnable, ERC20Permit, Pausable, EnsResolve {
  ...
  constructor(
    ...
    _pause();
    require(totalSupply() == 10000000 ether, "DEWO: incorrect distribution");
  }

  modifier onlyGovernance() {
    require(_msgSender() == governance, "DEWO: only governance can perform this action");
    _;
  }

  function changeTransferability(bool decision) public onlyGovernance {
    require(blockTimestamp() > canUnpauseAfter, "DEWO: cannot change transferability yet");
    if (decision) {
      _unpause();
    } else {
      _pause();
  ...
  function _beforeTokenTransfer(
    ...
    require(!paused() || allowedTransferee[from] || allowedTransferee[to], "DEWO: paused");
    require(to != address(this), "DEWO: invalid recipient");
  }
