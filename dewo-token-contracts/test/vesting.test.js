...
contract('Vesting', (accounts) => {
  ...
  before(async () => {
    ...
    dewo = await Dewo.new(governance, thirtyDays, [{ to: vestingExpectedAddr, amount: cap.toString() }])
    vesting = await Vesting.new(dewo.address, recipient, startTimestamp, cliffInMonths, durationInMonths)
  ...
  describe('#constructor', () => {
    it('should be initialized', async () => {
      ...
      const balance = await dewo.balanceOf(vesting.address)
  ...
  describe('#release', () => {
    ...
    it('should work if time has come', async () => {
      ...
      let balanceBefore = await dewo.balanceOf(recipient)
      await vesting.release()
      let balanceAfter = await dewo.balanceOf(recipient)

      const monthAfterCliff = cliff.add(MONTH)
      await vesting.setFakeTimestamp(monthAfterCliff)

      balanceBefore = await dewo.balanceOf(recipient)
      await vesting.release()
      balanceAfter = await dewo.balanceOf(recipient)
