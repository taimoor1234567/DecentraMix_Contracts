...
const DeMix = artifacts.require('./DeMixTokens.sol')
...
contract('DeMixTokens', accounts => {
  ...
  describe('#constructor', () => {
    it('should initialize', async () => {
      const tokenFromContract = await demix.token()
  ...
  describe('#deposit', () => {
    it('should work', async () => {
      ...
      await token.approve(demix.address, tokenDenomination)

      let { logs } = await demix.deposit(commitment, { from: sender })
    ...
    it('should not allow to send ether on deposit', async () => {
      ...
      let error = await demix.deposit(commitment, { from: sender, value: 1e6 }).should.be.rejected
  ...
  describe('#withdraw', () => {
    it('should work', async () => {
      ...
      await token.approve(demix.address, tokenDenomination, { from: user })
      // Uncomment to measure gas usage
      // let gas = await demix.deposit.estimateGas(toBN(deposit.commitment.toString()), { from: user, gasPrice: '0' })
      // console.log('deposit gas:', gas)
      await demix.deposit(toFixedHex(deposit.commitment), { from: user, gasPrice: '0' })
      ...
      const balanceDeMixBefore = await token.balanceOf(demix.address)
      ...
      const { logs } = await demix.withdraw(proof, ...args, { value: refund, from: relayer, gasPrice: '0' })

      const balanceDeMixAfter = await token.balanceOf(demix.address)
      ...
      balanceDeMixAfter.should.be.eq.BN(toBN(balanceDeMixBefore).sub(toBN(tokenDenomination)))
    ...
    it('should reject with wrong refund value', async () => {
      ...
      let { reason } = await demix.withdraw(proof, ...args, { value: 1, from: relayer, gasPrice: '0' }).should.be.rejected
      reason.should.be.equal('Incorrect refund amount received by the contract')


      ;({ reason } = await demix.withdraw(proof, ...args, { value: toBN(refund).mul(toBN(2)), from: relayer, gasPrice: '0' }).should.be.rejected)
      reason.should.be.equal('Incorrect refund amount received by the contract')
    })
    ...
    it.skip('should work with REAL USDT', async () => {
      ...
      await usdtToken.approve(demix.address, tokenDenomination, { from: user })
      console.log('approve done')
      const allowanceUser = await usdtToken.allowance(user, demix.address)
      ...
      const balanceDeMixBefore = await usdtToken.balanceOf(demix.address)
      ...
      const ethBalanceRecieverBefore = await web3.eth.getBalance(toFixedHex(recipient, 20))
      let isSpent = await demix.isSpent(input.nullifierHash.toString(16).padStart(66, '0x00000'))
      isSpent.should.be.equal(false)
      ...
      const balanceDeMixAfter = await usdtToken.balanceOf(demix.address)
      ...
      logs[0].args.fee.should.be.eq.BN(feeBN)
      isSpent = await demix.isSpent(input.nullifierHash.toString(16).padStart(66, '0x00000'))
      isSpent.should.be.equal(true)
    })
