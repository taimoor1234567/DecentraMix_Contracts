...
const DeMix = artifacts.require('./DeMixFactory.sol')
...
contract('DeMixFactory', accounts => {
  let demix
  ...
  describe('#constructor', () => {
    it('should initialize', async () => {
      const etherDenomination = await demix.denomination()
  ...
  describe('#deposit', () => {
    it('should emit event', async () => {
      ...
      let { logs } = await demix.deposit(commitment, { value, from: sender })
      ...
      ({ logs } = await demix.deposit(commitment, { value, from: accounts[2] }))
    ...
    it('should throw if there is a such commitment', async () => {
      ...
      await demix.deposit(commitment, { value, from: sender }).should.be.fulfilled
      const error = await demix.deposit(commitment, { value, from: sender }).should.be.rejected
  ...
  describe('#withdraw', () => {
    it('should work', async () => {
      ...
      // let gas = await demix.deposit.estimateGas(toBN(deposit.commitment.toString()), { value, from: user, gasPrice: '0' })
      // console.log('deposit gas:', gas)
      await demix.deposit(toFixedHex(deposit.commitment), { value, from: user, gasPrice: '0' })
      ...
      const balanceDeMixBefore = await web3.eth.getBalance(demix.address)
      ...
      let isSpent = await demix.isSpent(toFixedHex(input.nullifierHash))
      isSpent.should.be.equal(false)

      // Uncomment to measure gas usage
      // gas = await demix.withdraw.estimateGas(proof, publicSignals, { from: relayer, gasPrice: '0' })
      ...
      const { logs } = await demix.withdraw(proof, ...args, { from: relayer, gasPrice: '0' })

      const balanceDeMixAfter = await web3.eth.getBalance(demix.address)
      ...
      balanceDeMixAfter.should.be.eq.BN(toBN(balanceDeMixBefore).sub(toBN(value)))
      ...
      isSpent = await demix.isSpent(toFixedHex(input.nullifierHash))
    ...
    it('should prevent double spend', async () => {
      const deposit = generateDeposit()
      await tree.insert(deposit.commitment)
      await demix.deposit(toFixedHex(deposit.commitment), { value, from: sender })
      ...
      await demix.withdraw(proof, ...args, { from: relayer }).should.be.fulfilled
      const error = await demix.withdraw(proof, ...args, { from: relayer }).should.be.rejected
    ...
    it('should reject with tampered public inputs', async () => {
      ...
      let error = await demix.withdraw(proof, ...incorrectArgs, { from: relayer }).should.be.rejected
      ...
      error = await demix.withdraw(proof, ...incorrectArgs, { from: relayer }).should.be.rejected
      error.reason.should.be.equal('Invalid withdraw proof')
      ...
      await demix.withdraw(proof, ...args, { from: relayer }).should.be.rejected

      // should work with original values
      await demix.withdraw(originalProof, ...args, { from: relayer }).should.be.fulfilled
    })
  ...
  describe('#changeOperator', () => {
    it('should work', async () => {
      let operator = await demix.operator()
      operator.should.be.equal(sender)

      const newOperator = accounts[7]
      await demix.changeOperator(newOperator).should.be.fulfilled

      operator = await demix.operator()
    ...
    it('cannot change from different address', async () => {
      ...
      const newOperator = accounts[7]
      const error = await demix.changeOperator(newOperator, { from:  accounts[7] }).should.be.rejected
  ...
  describe('#updateVerifier', () => {
    it('should work', async () => {
      ...
      await demix.updateVerifier(newVerifier).should.be.fulfilled

      const verifier = await demix.verifier()
    ...
    it('cannot change from different address', async () => {
      ...
      const newVerifier = accounts[7]
      const error = await demix.updateVerifier(newVerifier, { from:  accounts[7] }).should.be.rejected
      error.reason.should.be.equal('Only operator can call this function.')
  ...
  describe('#isSpent', () => {
    it('should work', async () => {
      ...
      await demix.deposit(toFixedHex(deposit1.commitment), { value, gasPrice: '0' })
      await demix.deposit(toFixedHex(deposit2.commitment), { value, gasPrice: '0' })
      ...
      await demix.withdraw(proof, ...args, { from: relayer, gasPrice: '0' })
      ...
      const spentArray = await demix.isSpentArray([nullifierHash1, nullifierHash2])
