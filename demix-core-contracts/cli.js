...
let web3, demix, circuit, proving_key, groth16, erc20, senderAccount, netId
...
const networks = { '56': 'bsc', '97': 'testnet', '25': 'cronos',  '250': 'fantom', '137': 'matic', '1': 'mainnet', '43114': 'avax', }
...
async function deposit({ currency, amount }) {
  ...
  const noteString = `demix-${currency}-${amount}-${netId}-${note}`
  console.log(`Your note: ${noteString}`)
  if (currency === 'eth') {
    await printETHBalance({ address: demix._address, name: 'DeMix' })
    await printETHBalance({ address: senderAccount, name: 'Sender account' })
    const value = isLocalRPC ? ETH_AMOUNT : fromDecimals({ amount, decimals: 18 })
    console.log('Submitting deposit transaction')
    await demix.methods.deposit(toHex(deposit.commitment)).send({ value, from: senderAccount, gas:2e6 })
    await printETHBalance({ address: demix._address, name: 'DeMix' })
    await printETHBalance({ address: senderAccount, name: 'Sender account' })
  } else { // a token
    await printERC20Balance({ address: demix._address, name: 'DeMix' })
    ...
    const allowance = await erc20.methods.allowance(senderAccount, demix._address).call({ from: senderAccount })
    console.log('Current allowance is', fromWei(allowance))
    if (toBN(allowance).lt(toBN(tokenAmount))) {
      console.log('Approving tokens for deposit')
      await erc20.methods.approve(demix._address, tokenAmount).send({ from: senderAccount, gas:1e6 })
    }

    console.log('Submitting deposit transaction')
    await demix.methods.deposit(toHex(deposit.commitment)).send({ from: senderAccount, gas:2e6 })
    await printERC20Balance({ address: demix._address, name: 'DeMix' })
    await printERC20Balance({ address: senderAccount, name: 'Sender account' })
  }
 ...
 * Download deposit events from the demix, reconstructs merkle tree, finds our deposit leaf
 * in it and generates merkle proof
 * @param deposit Deposit object
 */
async function generateMerkleProof(deposit) {
  // Get all deposit events from smart contract and assemble merkle tree from them
  console.log('Getting current state from DecentraMix contract')
  const events = await demix.getPastEvents('Deposit', { fromBlock: 0, toBlock: 'latest' })
  const leaves = events
  ...
  const isValidRoot = await demix.methods.isKnownRoot(toHex(await tree.root())).call()
  const isSpent = await demix.methods.isSpent(toHex(deposit.nullifierHash)).call()
...
async function withdraw({ deposit, currency, amount, recipient, relayerURL, refund = '0' }) {
  ...
  if (relayerURL) {
    ...
    try{
      const relay = await axios.post(relayerURL + '/relay', { contract: demix._address, proof, args })
      if (netId === 1 || netId === 42) {
    ...
    await demix.methods.withdraw(proof, ...args).send({ from: senderAccount, value: refund.toString(), gas: 1e6 })
...
/**
 * Parses DecentraMix.io note
 * @param noteString the note
 */
function parseNote(noteString) {
  const noteRegex = /demix-(?<currency>\w+)-(?<amount>[\d.]+)-(?<netId>\d+)-0x(?<note>[0-9a-fA-F]{124})/g
...
async function init({ rpc, noteNetId, currency = 'dai', amount = '100' }) {
  let contractJson, erc20ContractJson, erc20demixJson, demixAddress, tokenAddress
  // TODO do we need this? should it work in browser really?
  if (inBrowser) {
    // Initialize using injected web3 (Metamask)
    // To assemble web version run `npm run browserify`
    web3 = new Web3(window.web3.currentProvider, null, { transactionConfirmationBlocks: 1 })
    contractJson = await (await fetch('build/contracts/DeMixFactory.json')).json()
    ...
    contractJson = require('./build/contracts/DeMixFactory.json')
    ...
    erc20demixJson = require('./build/contracts/DeMixTokens.json')
  }
  ...
  if (isLocalRPC) {
    demixAddress = currency === 'eth' ? contractJson.networks[netId].address : erc20demixJson.networks[netId].address
    ...
    try{
      demixAddress = config.deployments[`netId${netId}`][currency].instanceAddress[amount]
      if (!demixAddress) {
        throw new Error()
      }
      tokenAddress = config.deployments[`netId${netId}`][currency].tokenAddress
    } catch(e) {
      console.error('There is no such demix instance, check the currency and amount you provide')
      process.exit(1)
    }
  }
  demix = new web3.eth.Contract(contractJson.abi, demixAddress)
...
async function main() {
  if (inBrowser) {
    ...
    program
      ...
      .description('Submit a deposit of specified currency and amount from default eth account and return the resulting note. The currency is one of (ETH|DAI|cDAI|USDC|cUSDC|USDT). The amount depends on currency, see config.js file or visit https://decentramix.io.')
