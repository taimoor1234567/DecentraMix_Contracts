...
const { getDeMixTrees, txManager, getProvider } = require('./singletons')
const { action, getExplorer, poseidonHash, poseidonHash2, toFixedHex } = require('./utils')
const ethers = require('ethers')
const BigNumber = ethers.BigNumber
const { parseUnits } = ethers.utils
const demixTrees = require('demix-trees-contracts')
...
async function updateTree(committedEvents, pendingEvents, type) {
  ...
  const root = toFixedHex(await getDeMixTrees()[rootMethod]())
  ...
  while (pendingEvents.length >= INSERT_BATCH_SIZE) {
    ...
    const { input, args } = demixTrees.batchTreeUpdate(tree, chunk)
    const proof = await demixTrees.prove(input, './snarks/BatchTreeUpdate')

    console.log('Sending update tx')
    const method = type === action.DEPOSIT ? 'updateDepositTree' : 'updateWithdrawalTree'
    const txData = await getDeMixTrees().populateTransaction[method](proof, ...args, { gasPrice: parseUnits(GAS_PRICE, 'gwei') })
