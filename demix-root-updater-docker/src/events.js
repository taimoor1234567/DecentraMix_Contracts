const { getDeMixTrees, redis, getProvider } = require('./singletons')
...
async function getDeMixTreesEvents(type, fromBlock, toBlock) {
  const eventName = type === action.DEPOSIT ? 'DepositData' : 'WithdrawalData'
  const events = await getProvider().getLogs({
    address: getDeMixTrees().address,
    topics: getDeMixTrees().filters[eventName]().topics,
    fromBlock,
    toBlock,
  })
  return events
    .map((e) => {
      const { instance, hash, block, index } = getDeMixTrees().interface.parseLog(e).args
...
async function getEventsWithCache(type) {
  ...
  const newEvents = await getDeMixTreesEvents(type, lastBlock, currentBlock)
...
async function getPendingEventHashes(type, from, to) {
  try {
    const calls = []
    const target = (await getDeMixTrees()).address
...
async function getEvents(type) {
  ...
  const committedCount = (await getDeMixTrees()[committedMethod]()).toNumber()

  const pendingLengthMethod = type === action.DEPOSIT ? 'depositsLength' : 'withdrawalsLength'
  const pendingLength = (await getDeMixTrees()[pendingLengthMethod]()).toNumber()
  ...
  if (pendingEvents.some((e) => e === undefined)) {
    ...
    throw new Error('Tree contract expects unknown demix event')
  }
