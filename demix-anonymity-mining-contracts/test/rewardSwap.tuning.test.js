...
contract.skip('RewardSwap simulation', (accounts) => {
  ...
  async function exchange(points, verbose = true) {
    ...
    const poolSize = await rewardSwap.dewoVirtualBalance()
    if (verbose) {
      console.log(
        `Exchanged ${points} points for ${fromWei(
          balanceAfter.sub(balanceBefore),
        )} DEWO. Remaining in pool ${fromWei(poolSize)} DEWO`,
      )
    }
  }
  ...
  describe('Swap Simulations', () => {
    it('init', async () => {
      ...
      console.log('Virtual balance', fromWei(await rewardSwap.dewoVirtualBalance()))
    })
    ...
    it('rates', async () => {
      ...
      for (let i = 0; i < 18; i++) {
        console.log(
          `Expected return for 10^${i} points: ${fromWei(await rewardSwap.getExpectedReturn(k))} DEWO`,
        )
        k = k.mul(toBN(10))
      }
    })
