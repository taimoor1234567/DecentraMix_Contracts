...
const DeMixTrees = artifacts.require('DeMixTreesMock')
const DeMixTreesV1 = artifacts.require('DeMixTreesV1Mock')
...
async function registerNote(note, demixTrees) {
  await demixTrees.register(
    note.instance,
...
contract('Miner', (accounts) => {
  ...
  let rewardSwap
  let demixTrees
  const demix = '0x3535249DFBb73e21c2aCDC6e42796d920A0379b7'
  ...
  before(async () => {
    ...
    const demixTreesV1 = await DeMixTreesV1.new(
      0,
      0,
    ...
    demixTrees = await DeMixTrees.new(operator, demixTreesV1.address, {
      depositsFrom: 0,
      depositsStep: 0,
      withdrawalsFrom: 0,
      withdrawalsStep: 0,
    })
    await demixTrees.initialize(operator, verifier)
    ...
    miner = await Miner.new(
      ...
      toFixedHex(emptyTree.root()),
      [{ instance: demix, value: RATE.toString() }],
    )
    ...
    for (const note of notes) {
      const { depositLeaf, withdrawalLeaf } = await registerNote(note, demixTrees)
    ...
    await demixTrees.updateRoots(toFixedHex(depositTree.root()), toFixedHex(withdrawalTree.root()))

    const anotherWeb3 = new AnotherWeb3(web3.currentProvider)
    contract = new anotherWeb3.eth.Contract(miner.abi, miner.address)
    const demixTreesContract = new anotherWeb3.eth.Contract(demixTrees.abi, demixTrees.address)
  ...
  describe('#constructor', () => {
    it('should initialize', async () => {
      ...
      const rateFromContract = await miner.rates(demix)
  ...
  describe('#Note.fromString()', () => {
    it('should work', () => {
      const note = Note.fromString(
        'demix-eth-1-1-0x3a1f1e0e10b22b15ed8208bc810dd5564d564fd7930874db4d7d58870deb72978fb5fccae2f1554cac71e2cff85f9c8908295647adf2b443a4dd93635d8d',
      ...
      const note1 = Note.fromString(
        'demix-eth-1-5-0x00b9787ae8b877cc612acfc3d46eb1303c618ef386cb4383b75be5b300ac9b94818f3f7a589b4667612e879dbfd44198231c86aaa412a3770fd86d8fc045',
        '0x8b3f5393bA08c24cc7ff5A66a832562aAB7bC95f',
        10,
        15,
      )
  ...
  describe('#reward', () => {
    ...
    it('should send fee to relayer', async () => {
      ...
      await timeReset()
      let expectedFeeInDewo = await rewardSwap.getExpectedReturn(fee)
      let relayerBalanceBefore = await dewo.balanceOf(relayer)
      await miner.reward(claim.proof, claim.args)
      let relayerBalanceAfter = await dewo.balanceOf(relayer)
      relayerBalanceAfter.should.be.eq.BN(relayerBalanceBefore.add(expectedFeeInDewo))
      ...
      await timeReset()
      const expectedAmountInDewo = await rewardSwap.getExpectedReturn(amount)
      expectedFeeInDewo = await rewardSwap.getExpectedReturn(amount.add(fee))
      expectedFeeInDewo = expectedFeeInDewo.sub(expectedAmountInDewo)

      relayerBalanceBefore = await dewo.balanceOf(relayer)
      const recipientBalanceBefore = await dewo.balanceOf(recipient)
      await miner.withdraw(withdrawal.proof, withdrawal.args)
      const recipientBalanceAfter = await dewo.balanceOf(recipient)
      relayerBalanceAfter = await dewo.balanceOf(relayer)

      recipientBalanceAfter.should.be.eq.BN(recipientBalanceBefore.add(expectedAmountInDewo))
      relayerBalanceAfter.sub(relayerBalanceBefore).sub(expectedFeeInDewo).should.be.lt.BN(delta)
    })
  ...
  describe('#withdraw', () => {
    ...
    it('should work', async () => {
      ...
      balanceAfter.should.be.eq.BN(balanceBefore.add(expectedAmountInDewo))
    ...
    it('should reject for unfair amount', async () => {
      ...
      await timeReset()
      const balanceBefore = await dewo.balanceOf(recipient)
      const expectedAmountInDewo = await rewardSwap.getExpectedReturn(amountToWithdraw)
      await miner.withdraw(proof, args)
      const balanceAfter = await dewo.balanceOf(recipient)
      balanceAfter.should.be.eq.BN(balanceBefore.add(expectedAmountInDewo))
    })
  ...
  describe('#setRates', () => {
    it('should reject for invalid rates', async () => {
      ...
      await miner
        .setRates([{ instance: demix, value: bigNum.toString() }], { from: governance })
