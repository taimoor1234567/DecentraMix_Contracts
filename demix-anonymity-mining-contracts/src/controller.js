...
class Controller {
  constructor({ contract, demixTreesContract, merkleTreeHeight, provingKeys, groth16 }) {
    this.merkleTreeHeight = Number(merkleTreeHeight)
    this.provingKeys = provingKeys
    this.contract = contract
    this.demixTreesContract = demixTreesContract
  ...
  async _fetchEvents(eventName) {
    const events = await this.demixTreesContract.getPastEvents(eventName, {
      fromBlock: 0,
      toBlock: 'latest',
    })
    return events
   ...
   * @param {{instance: String, hash: String, block: Number, index: Number}[]} depositDataEvents An array of deposit objects from demixTrees contract. hash = commitment
   * @param {{instance: String, hash: String, block: Number, index: Number}[]} withdrawalDataEvents An array of withdrawal objects from demixTrees contract. hash = nullifierHash
   */
  async reward({
    account,
    note,
