...
contract('RelayerRegistry', (accounts) => {
  ...
  before(async () => {
    dewo = await Dewo.new()
    await dewo.transfer(accounts[5], '1000')
    registry = await RelayerRegistry.new(accounts[0], dewo.address)
  ...
  describe('#add', () => {
    ...
    it('should charge tokens', async () => {
      const relayerBalanceBefore = await dewo.balanceOf(relayer1)
      relayerBalanceBefore.should.eq.BN(toBN(1000))
      const registryBalanceBefore = await dewo.balanceOf(registry.address)
      registryBalanceBefore.should.eq.BN(toBN(0))

      const stake = toBN(1000)
      await dewo.approve(registry.address, stake, { from: relayer1 })
      await registry.setStake(stake)
      await registry.add(relayer1)

      const relayerBalanceAfter = await dewo.balanceOf(relayer1)
      relayerBalanceAfter.should.eq.BN(toBN(0))
      const registryBalanceAfter = await dewo.balanceOf(registry.address)
